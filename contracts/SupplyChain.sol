...
contract SupplyChain {
  ...
  modifier forSale(uint _sku) {
    require(items[_sku].state == SupplyChain.State.ForSale);
    _;
  }
  // modifier sold(uint _sku) 
  modifier sold(uint _sku) {
    require(items[_sku].state == SupplyChain.State.Sold);
    _;
  }
  // modifier shipped(uint _sku) 
  modifier shipped(uint _sku) {
    require(items[_sku].state == SupplyChain.State.Shipped);
    _;
  }
  // modifier received(uint _sku) 
  modifier received(uint _sku) {
    require(items[_sku].state == SupplyChain.State.Received);
    _;
  }
